    ...
    document.getElementById("search").addEventListener("keydown", function (event) {
        if (event.key === "Enter") {
            ...
            if (activeContent) {
                ...
                // set content
                activeContent.innerHTML = "<div id=\"container\"><iframe id=\"app\"></iframe></div>"
                ...
                fetch('https://axiomhub.net/main.php?url=' + query_fixed)
.then(response => {
    if (!response.ok) {
...
.then(data => {
    ...
    if (iframe.contentDocument) {
        iframe.contentDocument.body.innerHTML = data; // Using srcdoc might be better, but setting innerHTML for demonstration
        proxy(iframe);
    }
})
            ...
            if (tabs[i].id == currentTabId) {
                ...
                if (query.startsWith("glitch://")) {
                    ...
                    if (hit == 1){
                        ...
                        if (target == "config"){
                            activeContent.innerHTML = "No config yet"
                        }
                        if (target == "error"){
